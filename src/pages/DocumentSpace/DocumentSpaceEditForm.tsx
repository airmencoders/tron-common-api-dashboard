...
export default function DocumentSpaceEditForm(props: DocumentSpaceEditFormProps) {
  const formState = useHookstate<DocumentSpaceInfoDto>(props.documentSpace ?? { name: '' });

  formState.attach(Validation);  
  formState.attach(Initial);
  formState.attach(Touched);

  Validation(formState.name).validate(name => validateDocSpaceName(name ?? ''), 'Invalid Document Space Name', 'error');
  ...
  return (
    <Form onSubmit={submitForm}>
      ...
      >
        <Label htmlFor="new-space-name__input" error={!Validation(formState).valid()}>Space Name</Label>
        <TextInput
